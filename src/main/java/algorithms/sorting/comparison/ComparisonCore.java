...
public class ComparisonCore {
    ...
    /**
     * 二分搜索：
     * 假设在[begin, end) 范围内搜索某个元素v，先去找中间元素，mid == (begin + end) / 2       （搜索是begin左包含，end右不包含，叫做左闭右开，end是最后一个位置索引 + 1，m = mid索引的元素值）
     * 如果 v < m，就在左边范围搜索，在[begin, mid)范围内二分搜索      （下次搜索不会有到mid位置，mid被排除了，右不包含）
     * 如果 v > m，就在右边范围搜索，在[mid + 1, end)范围内二分搜索
     * 如果 v == m，就找到了结束
     * 如果 begin == end，则代表已经是没有元素的位置了，没有元素可以搜索，没搜索到     （end是不包含的位置，begin也等于，则代表已经在边界外了）
     *
     *
