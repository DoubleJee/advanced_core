...
public class DPCore {
     ...
     *
     * 1.定义状态   （状态是原问题、子问题的解）
     *  比如定义 dp(n) 的含义                 （例如：fib(n) = 第n项斐波那契数、 coins(n) = 凑够n分钱所需要的最小硬币数， 就是原问题、子问题的解）
     *
     * 2.设置初始状态 (边界)
     *  比如设置 dp(0)、dp(1) 的值            （例如：fib(1)、fib(2) = 1、 coins(25)、coins(5) = 1， 就是设置初始解，可以轻易推出来的子问题状态解）
     *
     * 3.确定状态转移方程
     *  比如确定 dp(n) 和 dp(n - 1) 的关系    （例如：fib(n) = fib(n - 1) + fib(n - 2)、 dp(n) = min{dp(n - 25), dp(n - 5)} + 1， 就是由旧状态得到新状态的方程，由之前的解求出新解，最后子问题的解得出原问题的解，如可以从fib(1) 推到 fib(n)的fib方程）
     *
     *
