...
public class DPCore {
     ...
     *
     * 3.确定状态转移方程
     *  比如确定 dp(i) 和 dp(i - 1) 的关系    （例如：fib(n) = fib(n - 1) + fib(n - 2)、 dp(n) = min{dp(n - 25), dp(n - 5)} + 1， 就是由旧状态得到新状态的方程，新状态怎么由旧状态推出来，由之前的解求出新解，最后子问题的解得出原问题的解，如可以从fib(1) 推到 fib(n)的fib方程）
     *
     *
