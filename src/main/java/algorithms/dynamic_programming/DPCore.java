...
public class DPCore {
     ...
     *
     * 3.确定状态转移方程
     *  比如确定 dp(i) 和 dp(i - 1) 的关系    （例如：fib(n) = fib(n - 1) + fib(n - 2)、 dp(n) = min{dp(n - 25), dp(n - 5)} + 1， 就是由旧状态得到新状态的方程，新状态怎么由旧状态推出来，子状态怎么转移到比较大的状态，由之前的解求出新解，最后子问题的解得出原问题的解，如可以从fib(1) 推到 fib(n)的fib方程）
     *
     *
     * 可以用动态规划解决的问题，通常具备两个特点：
     * 1.最优子结构（最优化原理）：通过求解子问题的最优解，可以获得原问题的最优解
     * 2.无后效性：
     *  某阶段的状态一旦确定，则此后过程的演变不再受此往前各个状态及决策的影响       （每个阶段只看自己的前一个阶段状态，不去看再前面的那些状态，此后关注此，不关注此往前，此就是此往前的各种决策结果的代表，未来与过去无关）
