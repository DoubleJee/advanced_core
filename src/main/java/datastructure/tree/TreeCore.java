...
public class TreeCore {
     ...
     * 4.比较矮
     *
     * m阶B树，（m >= 2 至少大于等于2）代表这颗B树的子节点最多的为m，一棵B树的一个节点有6个节点，是最多的节点，就是6阶B树
     *
     * ceil为向上取整
     * 根节点元素个数x最大为 1 <= x <= m - 1，也就是1 ~ m-1 个
     * 非根节点元素个数x最大为 ceil(m/2) <= x <= m - 1，也就是 m/2 ~ m-1 个
     *
     * 节点的最大子节点数y
     * 根节点的最大子节点数y  2 <= y <= m 也就是 2 ~ m个
     * 非根节点的最大节点数y  ceil(m/2) <= y <= m 也就是 m/2 ~ m个
     * 也就是 y = x + 1
     */
