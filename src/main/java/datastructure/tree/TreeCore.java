...
public class TreeCore {
     ...
     *
     * 一开始所有的元素都独占一个集合，它们分别是各个集合的根节点，然后进行合并、查找  （这里的集合，不是指Set这种数据结构，是广义的数据集合，数据集）
     * 每个集合只有一个根节点，根节点代表这个集合
     * 集合的元素都有父节点，父节点指向自己的，是根节点  （最顶层，也代表没父节点）
     ...
     * 有两种实现思路：
     * Quick Find（快查并查集）：
     * 构建出来的并查集，每个集合最多只有两层，所有元素的父节点都直接是根节点
     *
     * 查找（Find） 操作的时间复杂度是O(1)，因为直接拿到元素的父节点，也就是根节点（根节点就代表这个集合）
     * 合并（Union）操作的时间复杂度是O(n)，因为合并时，要改变另外一个集合的所有元素的 父节点 为，要合并元素苏所在集合的 根节点
     *
     * Quick Union（快合并查集）
     *
     *
