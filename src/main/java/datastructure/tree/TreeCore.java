...
public class TreeCore {
     ...
     * 操作公式为 r.left = rl.right; rl.right = r; （相对）root = rl;
     ...
     * 操作公式为 r.right = rr.left; rr.left = r; （相对）root = rr;
     * 也就是旋转节点的右子树 = 原右子树的左子树，原右子树的左子树 = 旋转节点，这棵子树的（相对）根节点 = 原右子树
     * 也要对应维护 每个参与变化节点的parent属性更新、ib和ibr的高度更新
     *
     * LR情况 因为是LR，所以需要找单向情况，L后面是R，不是单向，应该看R后面，后面没了，默认是一样的R情况，所以对R的那个子树进行RR左旋转。 也就是 先RR左旋转 再LL右旋转（双旋转）
