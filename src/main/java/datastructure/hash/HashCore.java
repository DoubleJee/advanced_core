...
public class HashCore {
     ...
     * 尽量让每个key的哈希值唯一、尽量让key的所有信息参与运算
     *
     * 整数直接拿整数值当hash值
     * 浮点数将内存里表示自己的bits，当成整数来读，读取的整数当hash值
     * Long利用高32bit和低32bit，混合计算出32bit的整数，当做hash值，（jdk做法是将高32bit与低32bit进行异或运算后，将32bit读成整数，= (int) bit ^ (bits >>> 32) )
     * Double转换成Long，然后同上
     *
     ...
     * 在添加元素时，可能会由单向链表转为红黑树来存储元素，比如当 哈希表容器 >= 64 并且 单向链表节点数大于8 ，转换为单向链表
